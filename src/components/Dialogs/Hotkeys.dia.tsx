...
export default class HotkeyDialog extends React.Component<IRecipeProps, { sections: any[], query: string }> {
    ...
    render() {
        ...
        const filterSection = (section) => {
            let childMatches = Object.keys(section.keys)
                .filter(filterItem.bind(this, section.keys)).length;
            return section.name.toLowerCase()
                .includes(this.state.query.toLowerCase()) || childMatches;
        }
            ...
            <Dialog
                ...
                <div style={{ width: 'auto', minWidth: '150px' }}>
                    <div className="space-between">
                        <Title text="Hot Keys" />
                        <Input value={this.state.query} placeholder="Search..." onChange={(query) => {
                    ...
                    >
                        {this.state.sections
                            ...
                            .map(section => {
                                return (
                                    <div className="list" key={section.name} style={{ display: 'flex', height: '100%', paddingBottom: '20px', flexDirection: 'column', marginBottom: '15px' }}>
