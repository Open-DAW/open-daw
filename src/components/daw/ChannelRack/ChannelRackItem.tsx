...
export default class ChannelRackItem extends ComponentBase<IRecipeProps, IRecipeState> {
    ...
    componentDidMount() {
        let patternMap = Array(this.props.octaves * 8).fill(0).map(() => '');
            ...
            octaves.push(
                <div key={i} className="octave">
                    {Array(4).fill(0).map((el, i) =>
                        <CRPatternKey onClick={(active) => this.setPatternActive(active, i)} />
                    )}
                    {Array(4).fill(0).map((el, i) =>
                        <CRPatternKey onClick={(active) => this.setPatternActive(active, i + 4)} color="red" />
                    )}
                    {Array(4).fill(0).map((el, i) =>
                        <CRPatternKey onClick={(active) => this.setPatternActive(active, i + 8)} />
                    )}
                    {Array(4).fill(0).map((el, i) =>
                        <CRPatternKey onClick={(active) => this.setPatternActive(active, i + 12)} color="red" />
                    )}
                </div>
            );
        }
        ...
        return (
            ...
            >
                <div
                    ...
                    onBlur={this.onBlur}
                    key={this.props.name}
                    ref={ref => this.ref = ref} data-tip={this.props.name} data-effect="solid" data-place="bottom"
